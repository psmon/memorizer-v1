...
# 개선지침
- 공유된 챗봇 대화내용중 Save As Memory가 잘작동하며 다음과 같이 개선하려고합니다.
  - 현재 대화/응답 한쌍의 내용만 메모리로 저장되며, 대화가 두쌍일시 BadRequest 오류가납니다.
     - 대화가 여러쌍일시에서도 메모리로 저장되도록 개선합니다. 
     - 메모리에 한꺼번에 저장하지말고 대화쌍별로 메모리로 각각 저장합니다.
     - 메모리가 모두 추가되고나면 이 메모리를 상호연결합니다.
